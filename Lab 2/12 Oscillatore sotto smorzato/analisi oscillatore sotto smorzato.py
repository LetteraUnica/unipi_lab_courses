...
C = unumpy.uarray(0.47e-6, 0.047e-6)
tempo, Vc = np.genfromtxt("datiharm3.txt", unpack = True, skip_header=1, skip_footer = 14)
tempo = tempo*10**-6
dt = np.ones(len(tempo))*3e-6
dVc = np.ones(len(Vc))
dVc_eff = dVc
p0 = (300, 0.02, 4300, np.pi/2, 489)
...
plt.figure()
plt.subplot(211)
plt.title("Oscillatore sotto-smorzato")
